Create one horizontal animation strip for Codex pet `sharki-desktop`, state `jumping`.

Repair requirement: the previous attempt failed because every pose stayed on the same foot baseline. Make vertical displacement unmistakable at final 192x208 size: frame 1 grounded crouch, frame 2 rising with both boots visibly above the original baseline, frame 3 clear airborne apex near the upper safe margin, frame 4 descending but still airborne, frame 5 grounded landing/recovery. Preserve one stable character scale; move the whole body vertically rather than merely changing limb poses. Both boots must have obvious empty chroma background below them in the airborne frames.
